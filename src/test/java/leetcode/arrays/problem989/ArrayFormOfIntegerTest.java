...
public class ArrayFormOfIntegerTest {
    ...
    void test_addToArrayForm_expect1234_whenKIs34() {
        int[] input = {1, 2, 0, 0};
        Assertions.assertEquals(classObj.addToArrayForm(input, 34), Arrays.asList(1, 2, 3, 4));
    }

    @Test
    void test_addToArrayForm_expect455_whenKIs181() {
        int[] input = {2, 7, 4};
        Assertions.assertEquals(classObj.addToArrayForm(input, 181), Arrays.asList(4, 5, 5));
    }

    @Test
    void test_addToArrayForm_expect1021_whenKIs806() {
        int[] input = {2, 1, 5};
        Assertions.assertEquals(classObj.addToArrayForm(input, 806), Arrays.asList(1, 0, 2, 1));
    }
}
